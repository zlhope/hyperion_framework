...
//		config.setServerURL(new URL("http://tlstage.telenav.com/lib/api/xmlrpc.php"));
	...
	public boolean uploadTestResult(String projectName, String testPlanName,
			String buildName,
			String resultStatus, String testcaseName,Connection connection)
			throws Exception {
		boolean successfulUpload = true;
		String tc_id = testcaseName.split("_")[1];
		int testCaseID = 0;
		if (tc_id != null) {
			int testPlanID = getTestPlanID(testPlanName, projectName);
			if (testPlanID != 0) {
				testCaseID = checkTestCase(tc_id, testPlanID, connection);
				if (testCaseID != 0) {
					...
					if (buildID != 0) {
						...
						if (uploadResult == 1) {
							System.out.println("The test execution result upload of test case: "
									+ testcaseName + " - " + testCaseID + " is successful!\n");
						} else {
							successfulUpload = false;
							System.out.println("The test execution result upload of test case: "
									+ testcaseName + " - " + testCaseID + " is fail!\n");
						}
					} else {
						successfulUpload = false;
			...
			successfulUpload = false;
			System.out.println("The relevant test case for custom field value - " + testcaseName +
					" does not exist!\n");
		}
	...
	private int checkTestCase(String tc_id, int testPlanID,
			Connection connection) {
		int uniqueTestCaseID = 0;
		ResultSet rs = null;
		String query = "select t2.`parent_id` from `testplan_tcversions` t1, nodes_hierarchy t2 where t1.`testplan_id`=" + String.valueOf(testPlanID) +
		               " and t1.`tcversion_id` = t2.`id` and t1.`tcversion_id` in (SELECT t.`id` FROM `tcversions` t WHERE t.`tc_external_id` ="+ tc_id +")";
